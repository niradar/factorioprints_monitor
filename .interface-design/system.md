...
- **Segmented pills (`.filters`):** the standard control for scoping a list - a query-param-driven `<a>` group in a `--slot` tray, active item `.on`. Used for the inbox status filter (`All` default+first · `Needs reply` · `Done`) and the Blueprints Δ-window switcher (`Today · 7d · 30d`). Reuse this component; don't invent a second tab/toggle style.
...
- **Table (Blueprints):** sortable headers, mono right-aligned numbers, orange `await` badge for >0 / quiet `-` for 0, clickable rows → detail. Sheds lower-priority columns (Comments, Last comment) under 820px. A **Δ-window column** ("new favourites" in the selected window) is driven by the segmented switcher in the toolbar: picking a window rescopes the column *and* sorts the table by it descending, so recent gains float to the top. `.delta` badge: green `up` / red `down` / muted `flat`; muted `-` when there's no snapshot old enough for the window (never a fake `+0`). A blueprint that first appears *within* the window (published after the baseline snapshot, on an account that already has older history) counts all its favourites as new (baseline 0) rather than showing `-` - only a brand-new account, whose first scan *is* the baseline, shows `-`. A one-line **movers summary** under the heading (`+41 across 9 blueprints since <date>`) labels the *real* baseline date, not the nominal window, so an off-by-a-day scan gap never misleads. The client-side pager (`table-pager.js`) reads the server's initial sort from `data-sort-col`/`data-sort-asc` so this default survives its takeover.
